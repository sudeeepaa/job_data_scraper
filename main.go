...
// searchJobsByTitle searches for jobs by title using substring matching
// the search is case-insensitive
func searchJobsByTitle(jobs []Job, searchTerm string) []Job {
	var results []Job

	// convert search term to lowercase for case-insensitive comparison
	lowerSearchTerm := strings.ToLower(searchTerm)

	// check each job title against the search term
	for _, job := range jobs {
		if strings.Contains(strings.ToLower(job.Title), lowerSearchTerm) {
...
func main() {
	...
	// search functionality to find jobs by title
	scanner := bufio.NewScanner(os.Stdin)
	fmt.Print("search for job titles (or press enter to skip): ")

	if scanner.Scan() {
		searchTerm := scanner.Text()

		// only perform search if user entered something
		if searchTerm != "" {
			results := searchJobsByTitle(jobs, searchTerm)

			fmt.Println()
			fmt.Printf("search results for '%s':\n", searchTerm)

			// if no jobs match the search term
			if len(results) == 0 {
				fmt.Println("  no matching jobs found")
			} else {
				// displaying all matching jobs
				for i, job := range results {
					fmt.Printf("  %d) %s at %s (%s)\n",
						i+1, job.Title, job.Company, job.Platform)

					// showing salary if available
					if job.Salary > 0 {
						fmt.Printf("     salary: $%d\n", job.Salary)
					} else {
						fmt.Println("     salary: not specified")
